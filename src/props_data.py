...
def construir(db: Path = DB, jugadores_csv: Path = JUGADORES_CSV,
    ...
    for player_norm, player_name, equipo, n_part, total_rem in rows_jug:
        cod = _codigo_nacion(equipo)
        tiros_pp = total_rem / n_part  # tiros por partido (titular ~90 min)
...
def main() -> None:
    ...
    print(f"  Conversion rate intl: {meta['conversion_rate_intl']:.3f}  "
          f"(1 gol cada {1/meta['conversion_rate_intl']:.1f} tiros)")
    print(f"  Escala tiros intl/club: {meta['escala_tiros_seleccion']:.3f}")
    print(f"  SOT rate default: {meta['sot_rate_default']:.3f}")
    print(f"  Shadow tiros/90: {meta['shadow_tiros_90']}")
    print("\n  Top remateadores con historial intl (>= 3 partidos):")
    top = sorted(datos["jugadores"].items(),
                 key=lambda x: x[1]["tiros_pp_intl"], reverse=True)[:10]
    for nombre, v in top:
        print(f"    {v['fuente_fbref'] or nombre:<30}  intl={v['tiros_pp_intl']:.2f}/p  "
              f"club={v['tiros_90_club']:.2f}/90  n={v['n_intl']}")
